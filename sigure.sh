...
if [ ${res_build} -eq 0 ]; then
    ...
    if [ "${zip_name}" != "*" ]; then
        ...
        mv --backup=t "${run_dir}/${source_dir}/out/target/product/${target_device}/${zip_name}.zip.md5sum" "${run_dir}/${zip_folder_name}/"
    else
        ...
        rm ${run_dir}/${zip_folder_name}/*-ota-*.zip
    fi
else
...
if [ "${tweet}" = "true" ]; then
    ...
    end_build_log=$(tail -2 "${run_dir}/${log_folder_name}/${res_build_str}/${log_file_name}.log" | head -1 | sed -r 's/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g' | sed 's/#//g' | sed 's/^[ ]*//g' | sed 's/ (hh:mm:ss)//g' | sed 's/ (mm:ss)//g' | sed 's/ seconds)/s/g' | sed 's/(//g' | sed 's/)//g' | sed 's/make failed to build some targets/make failed/g' | sed 's/make completed successfully/make successful/g')
